    ...
    return (
      <div>
        {this.state.companies.map(company =>
          <CompanyTable key={company.symbol}
                        company={company}
                        onDelete={() => this.setState({companies: this.state.companies.filter(c => c !== company)})}
                        onCalculate={this.state.companies.length > 1 ? () => this.calculateForCompany(company) : null}/>
        )}
        <button onClick={() => this.setState({searchPopupOpen: true})}>Add</button>
        {this.state.searchPopupOpen && <SelectSymbolModal open={this.state.searchPopupOpen}
                                                          onSelect={symbol => {
                                                            Promise.all([
        ...
        />}
        {this.state.calculateForCompany && <CompanyInfoModal company={this.state.calculateForCompany}
                                                             allCompanies={this.state.companies}
